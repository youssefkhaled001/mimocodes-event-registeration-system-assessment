<x-public-layout>
    <x-event-modal :events="$events">
        <div class="relative z-10 max-w-7xl mx-auto px-8 py-7">
            <!-- Header -->
            <header class="mb-20 animate-fadeInUp">
                ...
                <p class="text-lg font-light text-white/60 max-w-2xl leading-relaxed">
                    Discover and register for our curated collection of events. From workshops to conferences, find your
                    next opportunity to learn and connect.
                </p>
            </header>
            ...
            @if($events->count() > 0)
                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 mb-16">
                    @forelse($events as $index => $event)
                        <article
                            class="group relative bg-white/[0.02] backdrop-blur-sm border border-white/[0.08] rounded-xl overflow-hidden transition-all duration-500 hover:bg-white/[0.04] hover:border-cyan-500/20 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,255,255,0.05)] animate-fadeInUp delay-{{ min(($index + 1) * 50 + 50, 350) }}">
                            <!-- Top gradient line on hover -->
                            <div
                                class="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400">
                            </div>
                            ...
                            <div class="p-10">
                                ...
                                <div class="flex items-center gap-4 mb-6">
                                    <span
                                        class="inline-flex items-center gap-2 text-xs font-medium text-white/50 uppercase tracking-widest">
                                        <span class="w-1 h-1 rounded-full bg-cyan-400/60"></span>
                                        {{ $event->date_time->format('M d, Y • g:i A') }}
                                    </span>
                                    <span
                                        class="px-3 py-1 bg-cyan-500/10 border border-cyan-500/20 rounded text-[0.625rem] font-semibold text-cyan-400 uppercase tracking-widest">
                                        {{ $event->status }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-col gap-4 mb-8 pt-8 border-t border-white/5">
                                    <div class="flex items-start gap-4">
                                        <div
                                            class="w-9 h-9 rounded-lg bg-cyan-500/5 border border-cyan-500/10 flex items-center justify-center text-cyan-400 flex-shrink-0">
                                            📍
                                        </div>
                                        <div class="pt-1">
                                            <div
                                                class="text-[0.6875rem] text-white/40 uppercase tracking-widest font-medium mb-1">
                                                Location</div>
                                ...
                                <!-- Footer -->
                                <div class="mb-8">
                                    <div class="text-[0.6875rem] text-white/40 uppercase tracking-widest font-medium mb-2">Price
                                    </div>
                                    <div
                                        class="font-serif text-4xl font-bold {{ $event->price == 0 ? 'text-emerald-400' : 'text-cyan-400' }} leading-none">
                                        @if($event->price > 0)
                    ...
                    @empty
                        <div
                            class="text-center py-24 bg-white/[0.02] backdrop-blur-sm border border-white/[0.08] rounded-xl animate-fadeInUp">
                            <div class="text-6xl mb-8 opacity-30">○</div>
                            <h2 class="font-serif text-4xl font-semibold text-white mb-4 tracking-tight">No Events Available
                            </h2>
